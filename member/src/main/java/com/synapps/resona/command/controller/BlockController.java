...
import com.synapps.resona.command.service.BlockService;
...
public class BlockController {
  ...
  @ApiSuccessResponse(@SuccessCodeSpec(enumClass = MemberSuccessCode.class, code = "BLOCK_SUCCESS"))
  @ApiErrorSpec({
      @ErrorCodeSpec(enumClass = MemberErrorCode.class, codes = {"CANNOT_BLOCK_SELF", "ALREADY_BLOCKED"}),
  ...
  public ResponseEntity<SuccessResponse<Void>> blockMember(HttpServletRequest request,
    ...
    blockService.blockMember(memberDto.getId(), memberId);
    return ResponseEntity
        .status(MemberSuccessCode.BLOCK_SUCCESS.getStatus())
        .body(SuccessResponse.of(MemberSuccessCode.BLOCK_SUCCESS,
            createRequestInfo(request.getRequestURI())));
  }

  @Operation(summary = "사용자 차단 해제", description = "차단한 사용자를 해제합니다. (인증 필요)")
  @ApiSuccessResponse(@SuccessCodeSpec(enumClass = MemberSuccessCode.class, code = "UNBLOCK_SUCCESS"))
  @ApiErrorSpec({
      @ErrorCodeSpec(enumClass = MemberErrorCode.class, codes = {"NOT_BLOCKED"}),
      @ErrorCodeSpec(enumClass = MemberErrorCode.class, codes = {"MEMBER_NOT_FOUND"}),
      @ErrorCodeSpec(enumClass = AuthErrorCode.class, codes = {"TOKEN_NOT_FOUND", "INVALID_TOKEN"})
  })
  ...
  public ResponseEntity<SuccessResponse<Void>> unblockMember(HttpServletRequest request,
      ...
      @AuthenticationPrincipal UserPrincipal userPrincipal) {
    MemberDto memberDto = MemberDto.from(userPrincipal);
    blockService.unblockMember(memberDto.getId(), memberId);
    return ResponseEntity
        .status(MemberSuccessCode.UNBLOCK_SUCCESS.getStatus())
