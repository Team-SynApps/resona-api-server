...
class RecentCategoryFallbackStrategyTest {

    @InjectMocks
    private RecentCategoryFallbackStrategy strategy;

    @Mock
    private FeedReadRepository feedReadRepository;

    @Test
    @DisplayName("국가가 null이고 카테고리가 null이 아닐 때 지원한다.")
    void supports_CountryNullAndCategoryNotNull_ReturnsTrue() {
        // given
    ...
    @Test
    @DisplayName("카테고리별 최신 피드를 성공적으로 조회한다.")
    void findFeeds_Success() {
        // given
        PageRequest pageable = PageRequest.of(0, 10);
        FeedCategory category = FeedCategory.DAILY;
        FallbackStrategyContext context = new FallbackStrategyContext(pageable, category, null);
        FeedDocument feedDocument = mock(FeedDocument.class);
        List<FeedDocument> expectedFeeds = Collections.singletonList(feedDocument);

        when(feedReadRepository.findByCategoryOrderByCreatedAtDesc(category, pageable)).thenReturn(expectedFeeds);
